...
scrapers, regardless of what URL was requested (a listing, an event page,
anything) — this is deliberate, not scoped to any one page. Which challenge
a visitor meets depends on the exit country and on what the address has
recently been doing, and a narrow detector is how a rendered challenge gets
reported as an empty listing months later.

WHAT IS AND IS NOT EXERCISED ON OPENSEA
------------------------------------------
Nothing here ran against this site, and that is worth stating plainly rather
than leaving a reader to infer it. Twelve browser captures and three raw
fetches taken on 2026-09-16 contained ZERO captcha markers of any vendor and
...
family is never entitled to write is that a captcha here cannot be solved;
the one it is entitled to is "this repo implements X", which is what the
list below is (§19).

IMPLEMENTED HERE
----------------
  * reCAPTCHA v2 checkbox, v2 invisible and v3, through
    `RecaptchaV2TaskProxyless` / `RecaptchaV3TaskProxyless`;
  * reCAPTCHA ENTERPRISE, through `RecaptchaV2EnterpriseTaskProxyless` — a
    separate task type, because an enterprise widget solved as ordinary v2
    returns a token the site rejects;
  * Cloudflare Turnstile, through `TurnstileTaskProxyless`, including the
    Challenge-page form whose `sitekey`, `action`, `cData` and `chlPageData`
    exist only inside the one `turnstile.render` call the page makes — which
    is why the engines install an interception script on the context before
    any page script runs.

Spending stays NARROW while detection stays broad: `--solve-captcha
when-blocked` is the default and counts event links before paying,
`page_flow.SOLVES_PER_PAGE` caps a page at one purchase, and
`page_flow.STATE_POLICY` — not this file — decides which state is worth money
at all. A detection with no sitekey raises rather than buying a task the API
will reject.
...
# ordinary v2 checkbox — which is exactly what a sibling site's refusal
# looks like.
...
def detect_recaptcha_in_page(evaluate, page_url: str = "") -> Optional[CaptchaChallenge]:
    ...
    Version inference, and why it is deliberately conservative:
      ...
      * an interactive bframe present with NO size and NO render parameter is
        ...
        alone rules it out. A sibling site's denial page is exactly
    ...
    if render and render != "explicit" and render == info.get("sitekey"):
        ...
        #
        # This rung was absent, and its absence cost a real solve: a sibling
